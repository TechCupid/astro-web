...
const CustomerSignupFlow = ({ navigation }) => {
  const { API_BASE_URL, changeApiBaseUrl } = useApi();
  ...
  // NEW states for Terms & Privacy Policy Modals
  ...
  useEffect(() => {
    if (showCodeDropdown && searchInputRef.current) {
  ...
  const handleMobileSubmit = async () => {
    ...
    try {
      const response = await api.get(`${API_BASE_URL}/api/customers/details/mobile`, {
        params: { custMobileNo: mobileNumber },
      });
  ...
  // ✅ Add popup here
  setValidationMessage(`Saved mobileNumber in AsyncStorage: ${mobileNumber}`);
  setShowValidationModal(true);
  ...
  navigation.navigate('Main', { mobileNumber });
      ...
      if (error.response?.status === 404) {
        setOtpVisible(true);
        setTimer(60);
        setOtpInfoMsg(`OTP sent to +91 ${mobileNumber}`);
      } else {
...
const handleOtpVerification = async () => {
  ...
  try {
    const response = await api.get(`${API_BASE_URL}/api/customers/details/verifyOtp`, {
      params: { custMobileNo: mobileNumber, otp },
    });
...
// ✅ Add popup
setValidationMessage(`OTP verified ✅. Saved mobileNumber: ${mobileNumber}`);
setShowValidationModal(true);

      await AsyncStorage.setItem('APP_FOR', 'CUST');

      // ✅ CASE 1: Existing customer
      if (customerDetails?.custName || customerDetails?.isRegistered === true) {
        await AsyncStorage.setItem('isCustomerRegistered', 'true');

          await registerForPushNotificationsAsync(API_BASE_URL);

          // Add a short delay to guarantee backend update completes
await new Promise(resolve => setTimeout(resolve, 1500));

        // 👉 No FCM registration here (handled globally in App.js)
        navigation.navigate('Main', { mobileNumber });
      } 
      // ✅ CASE 2: New customer
      else {
  ...
  const handleResendOtp = () => {
    ...
    setOtpInfoMsg('');
    setOtpInfoMsg(`OTP has been resent to ${selectedCode.value} ${mobileNumber}`);
  };

  const isGetOtpEnabled = mobileNumber.length === 10 && agree;
      ...
      <View style={{ width: '100%' }}>
        <View style={styles.inputWrapper}>
         <TouchableOpacity
          ...
          onPress={toggleCodeDropdown}  // toggle dropdown instead of modal
        >
          <View style={styles.flagCircle}>
            <Text style={{ fontSize: 14 }}>{selectedCode.flag}</Text>
          </View>
          <Text style={{ marginLeft: 6, fontSize: 16 }}>{selectedCode.value}</Text>
          <Icon
        ...
        {showCodeDropdown && (
          <View style={[styles.modalBox1, { position: 'absolute', top: 50, left: 0, right: 0, height: 250, zIndex: 1000 }]}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.codeItem} onPress={() => onSelectCode(item)}>
        ...
        <>
          <View style={styles.checkboxRow}>
            ...
            <Text style={styles.checkboxText}>
              I agree to the{' '}
              <Text style={styles.link} onPress={() => setShowTermsModal(true)}>Terms</Text> &{' '}
              <Text style={styles.link} onPress={() => setShowPrivacyModal(true)}>Privacy Policy</Text>
            </Text>
          </View>

          {otpInfoMsg !== '' && <Text style={styles.otpInfoText}>{otpInfoMsg}</Text>}

          <TouchableOpacity
            style={[styles.getOtpButton, { backgroundColor: isGetOtpEnabled ? '#FF6600' : '#ccc' }]}
      ...
      {otpVisible && (
        <View style={styles.otpSection}>
          ...
          <TouchableOpacity onPress={handleResendOtp} disabled={timer > 0}>
            <Text style={[styles.linkText, timer > 0 && { color: '#ccc' }]}>Resend OTP via SMS</Text>
          </TouchableOpacity>

          <TouchableOpacity disabled={timer > 0}>
            <Text style={[styles.linkText, timer > 0 && { color: '#ccc' }]}>Get OTP via phone call</Text>
          </TouchableOpacity>
        </View>
      )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <View style={styles.contentWrapper}>
              ...
              <Text style={styles.modalMessage}>{setOtpInfoMsg}</Text>
            </View>
            <View style={styles.separator} />
            <LinearGradient colors={['#FC2A0D', '#FE9F5D']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.gradientBottom}>
              <TouchableOpacity onPress={() => setShowResendModal(false)} style={styles.okButton}>
                <Text style={styles.okText}>OK</Text>
              </TouchableOpacity>
            </LinearGradient>
          </View>
...
const styles = StyleSheet.create({
  ...
  input: { flex: 1, paddingVertical: 10, marginLeft: 5 },
  checkboxRow: { flexDirection: 'row', alignItems: 'center', marginTop: 20, marginBottom: 10 },
  checkboxText: { fontSize: Platform.OS === 'ios' ? 16 : 14, marginLeft: 8 },
  link: { color: 'blue' ,},
  getOtpButton: { borderRadius: 30, paddingVertical: 12, alignItems: 'center', marginTop: 10 },
  ...
  verifyButton: { backgroundColor: '#FF6600', marginTop: 20, paddingVertical: 10, paddingHorizontal: 40, borderRadius: 20 },
  verifyText: { color: '#fff', fontWeight: 'bold' },
  timerText: { marginTop: 15, fontWeight: '500' },
  timerColor: { color: '#FF6600' },
  linkText: { textDecorationLine: 'underline', color: '#007AFF', marginTop: 10 },
  otpInfoText: { color: '#FF6600', fontSize: 16, fontWeight: '500', marginBottom: 5 },
  modalOverlay: {
    flex: 1,
  ...
  modalBox: { width: '75%', backgroundColor: '#FFEFE5', borderRadius: 20, overflow: 'hidden', elevation: 10 },
      modalBox1: {
    width: '100%',
    backgroundColor: '#fff',
    borderRadius: 1,
...
modalContainer: {
  ...
  termsBox: { width: '90%', height: '80%', backgroundColor: '#fff', borderRadius: 15, padding: 20, elevation: 10 },
  termsContent: { fontSize: 14, color: '#333', lineHeight: 22, marginBottom: 10 , textAlign: 'justify',},
  closeButton: { backgroundColor: '#FF6600', padding: 12, borderRadius: 10, alignItems: 'center', marginTop: 15 },
